...
const AUMUpdateForm: React.FC = () => {
  const [formData, setFormData] = useState<AUMFormData>({
    clientTypes: Object.keys(clientTypeLabels).reduce(
      (acc, key) => ({
        ...acc,
        [key]: { count: '', amount: '' },
      }),
  ...
  useEffect(() => {
    ...
    const clientAUMTotal = Object.values(formData.clientTypes).reduce(
    ...
    const accountAUMTotal =
      (parseInt(formData.discretionaryAUM) || 0) +
      (parseInt(formData.nonDiscretionaryAUM) || 0);
    setTotalAccountAUM(accountAUMTotal.toString());

    // Calculate total accounts
    const accountsTotal =
      (parseInt(formData.discretionaryAccounts) || 0) +
    ...
    <Box sx={{ width: '100%', mt: 2 }}>
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body1" paragraph>
          Form ADV requires (and thus we are requesting) the following
          information regarding firm's AUM by client type and by account type.
          Since it is just different ways of breaking out AUM, the totals AUM in
          Section A must match that of Section B. All fields must be completed.
          Any dollar amount ($) fields must be whole positive integers without
          decimals, commas, or other non-numeric characters.
        </Typography>
      </Paper>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body1" paragraph>
          <strong>Clients vs. Accounts:</strong> Please be mindful of the
          difference between accounts and clients. For example, one husband and
          wife couple constitutes one single client for the purpose of your ADV
          filing. That client may have several accounts e.g. separate 401k
          accounts and general investment accounts. Information on the
          definition of a client, applicable to both U.S. and foreign advisers,
          can be found{' '}
          ...
          <Box key={key} sx={{ display: 'flex', mb: 2, gap: 2 }}>
            ...
            <Box sx={{ flex: 3 }}>
              <TextField
                ...
                onChange={(e) =>
                  handleClientTypeChange(key, 'count', e.target.value)
                }
            ...
            <Box sx={{ flex: 3 }}>
              <TextField
                ...
                onChange={(e) =>
                  handleClientTypeChange(key, 'amount', e.target.value)
                }
                InputProps={{
                  startAdornment: (
          ...
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  discretionaryAUM: e.target.value,
                }))
              }
              InputProps={{
                startAdornment: (
          ...
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  discretionaryAccounts: e.target.value,
                }))
              }
              InputProps={{
                endAdornment: (
                  <InputAdornment position="end">Accounts</InputAdornment>
                ),
              }}
            />
          </Box>
          ...
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  nonDiscretionaryAUM: e.target.value,
                }))
              }
              InputProps={{
                startAdornment: (
          ...
          <Box sx={{ flex: '1 1 45%', minWidth: '300px' }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  nonDiscretionaryAccounts: e.target.value,
                }))
              }
              InputProps={{
                endAdornment: (
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Box sx={{ mb: 3 }}>
          <Typography variant="subtitle1" gutterBottom>
            Please provide the AUM attributable to Non-U.S. clients to be
          ...
          <TextField
            ...
            onChange={(e) =>
              setFormData((prev) => ({
                ...prev,
                nonUSClientsAUM: e.target.value,
              }))
            }
            InputProps={{
              startAdornment: (
        ...
        <Box sx={{ mb: 3 }}>
          <Typography variant="subtitle1" gutterBottom>
            Please provide the AUM for each custodian to be disclosed in Form
            ADV Part 1A.
          </Typography>
          <Typography variant="body2" color="text.secondary" paragraph>
            • applicable for separately managed account AUM for custodians that
            hold 10% or more of your firm's separately managed account AUM
          </Typography>
          <Typography variant="body2" color="text.secondary" paragraph>
            ...
            <Box key={index} sx={{ display: 'flex', gap: 2, mb: 2 }}>
              <Box sx={{ flex: 1 }}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleCustodianChange(index, 'name', e.target.value)
                  }
                />
              </Box>
              <Box sx={{ flex: 1 }}>
                <TextField
                  ...
                  value={custodian.aum}
                  onChange={(e) =>
                    handleCustodianChange(index, 'aum', e.target.value)
                  }
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position="start">$</InputAdornment>
