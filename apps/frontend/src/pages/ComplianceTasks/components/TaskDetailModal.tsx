          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <AlertDialog.Title className="text-lg font-semibold">
                {task && (
                  <span
                    className={`${isPastDue ? 'text-red-600' : isCompleted ? 'text-green-600' : ''}`}
                  >
            ...
            {task && (
              <AlertDialog.Description className="sr-only">
                Compliance task details for {task.name}. Due on{' '}
                {format(new Date(task.due_date), 'MMM d, yyyy')}. Status:{' '}
              ...
              {task && (
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p
                      className={`font-medium ${isPastDue ? 'text-red-600' : ''}`}
                    >
                  ...
                  <div>
                    ...
                    <p
                      className={`font-medium ${isCompleted ? 'text-green-600' : isPastDue ? 'text-red-600' : 'text-blue-600'}`}
                    >
                      {isCompleted
                        ? 'Completed'
                        : isPastDue
